...
var babelPath = path.join(node_modules(), ".bin/babel");
var minifyPath = path.join(node_modules(), ".bin/minify");
...
function displayWelcome() {
    console.log("");
    console.log("electron-uglifier %s", module.exports.version);
    console.log("-----------------------");
    console.log("");
}
...
function readEnvironment() {
    ...
    configPath = path.join(repoDir, "electron_uglifier.json");

    try {
        config = JSON.parse(fs.readFileSync(configPath));
    } catch (error) {
        logError("Failed to parse electron_uglifier.json.");
        return false;
    }
...
function runPackager(platform, callback) {
    ...
    var options = {
        ...
        "arch": config.arch,
        "platform": platform,
        "appCopyright": config.versionString.CompanyName,
